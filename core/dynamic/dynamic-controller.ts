...
export class DynamicController {
    ...
    addSearchPaths() {
        ...
        router.get(this.path + "/search", MetaUtils.ensureLoggedIn(), (req, res) => {
    ...
    private addRoutesForAllSearch(map: ISearchPropertyMap, fieldsWithSearchIndex: any[]) {
        ...
        if (Utils.config().Config.ApplyElasticSearch) {
        ...
        if (searchFromDb) {
            router.get(this.path + "/search/" + map.key, MetaUtils.ensureLoggedIn(),(req, res) => {
    ...
    private isAuthorize(req: any, access: number): boolean {
        if (Utils.config().Security.isAutheticationEnabled == SecurityConfig.AuthenticationEnabled[SecurityConfig.AuthenticationEnabled.disabled]
            || Utils.config().Security.isAutheticationEnabled == SecurityConfig.AuthenticationEnabled[SecurityConfig.AuthenticationEnabled.enabledWithoutAuthorization])
            return true;
    ...
    private getFullBaseUrlUsingRepo(req, repoName): string {
        var fullbaseUr: string = "";
        fullbaseUr = req.protocol + '://' + req.get('host') + '/' + Utils.config().Config.basePath + '/' + repoName;
        return fullbaseUr;
    }
}

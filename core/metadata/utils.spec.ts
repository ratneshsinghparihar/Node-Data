...
describe('Utils function:', function () {
    ...
    describe('Metadata count ', function () {
        it('for student', () => {
            met = MetaUtils.getMetaData(models.student.prototype);
            expect(Enumerable.from(met).select(x => x.key).count()).toEqual(5);
        });

        it('for subject', () => {
            met = MetaUtils.getMetaData(models.subject.prototype);
            expect(Enumerable.from(met).select(x => x.key).count()).toEqual(3);
        });

        it('for teacher', () => {
            met = MetaUtils.getMetaData(models.teacher.prototype);
            expect(Enumerable.from(met).select(x => x.key).count()).toEqual(3);
        });

        it('for division', () => {
            met = MetaUtils.getMetaData(models.division.prototype);

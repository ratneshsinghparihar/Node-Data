...
export function generateSchema() {
    ...
    repositoryMetadata.forEach(x => {
        ...
        let schemaName = (<IDocumentParams>documentMeta[MetadataConstants.CLASSDECORATOR_PROPKEY].params).name;
        let schema = new DynamicSchema(documentMeta[MetadataConstants.CLASSDECORATOR_PROPKEY].target, (<IDocumentParams>x.metadata[0].params).name);
